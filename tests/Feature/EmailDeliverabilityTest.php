...
class EmailDeliverabilityTest extends TestCase
{
    ...
    public function test_track_click_decodes_html_entities_in_wrapped_urls(): void
    {
        $destination = 'https://smbgen.com/verify-email/8/hashvalue?expires=1775915628&amp;signature=abc123';
        $encoded = base64_encode($destination);

        $response = $this->get('/track/click/test-tracking-id?url='.urlencode($encoded));

        $response->assertRedirect('https://smbgen.com/verify-email/8/hashvalue?expires=1775915628&signature=abc123');
    }
