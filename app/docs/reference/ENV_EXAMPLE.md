...
**STRIPE_KEY** - Stripe publishable key (starts with `pk_test_` or `pk_live_`)

**STRIPE_SECRET** - Stripe secret key (starts with `sk_test_` or `sk_live_`)
...
# Optional: Resend API (alternative to SMTP)
# MAIL_MAILER=resend
