...
use App\Modules\SaasProductModule\Http\Controllers\AdminController;
use App\Modules\SaasProductModule\Http\Controllers\BillingController;
use App\Modules\SaasProductModule\Http\Controllers\DashboardController;
use App\Modules\SaasProductModule\Http\Controllers\OnboardingController;
use Illuminate\Support\Facades\Route;

// ─── PUBLIC ───────────────────────────────────────────────────────────────────
// 'web' must be explicit — loadRoutesFrom() runs outside the web middleware group
Route::middleware(['web'])->prefix('saas-product-module')->name('saasproductmodule.')->group(function () {
    Route::get('/plans', [BillingController::class, 'plans'])->name('billing.plans');
});

// ─── CUSTOMER (auth required) ─────────────────────────────────────────────────
Route::middleware(['web', 'auth', 'verified'])->prefix('saas-product-module')->name('saasproductmodule.')->group(function () {
...
Route::middleware(['web', 'auth', 'verified', 'companyAdministrator'])
    ->prefix('admin/saas-product-module')
    ->name('admin.saasproductmodule.')
    ->group(function () {
