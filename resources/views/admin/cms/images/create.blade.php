...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Upload CMS Images</h2>
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Add images for use in CMS content</p>
        </div>
    ...
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Upload Images</h3>
            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Select multiple images to upload at once</p>
        </div>
            ...
            <form id="uploadForm" action="{{ route('admin.cms.images.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <div id="uploadZone" class="relative">
                        <div class="flex justify-center px-6 pt-8 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-lg hover:border-blue-500 transition-all duration-200 cursor-pointer"
                             ondragover="handleDragOver(event)"
                             ondragleave="handleDragLeave(event)"
                             ondrop="handleDrop(event)">
                            <div class="space-y-3 text-center">
                                <div class="flex justify-center">
                                    <i class="fas fa-cloud-upload-alt text-gray-600 dark:text-gray-400 text-4xl mb-2"></i>
                                </div>
                                <div class="flex text-sm text-gray-600 dark:text-gray-400 justify-center items-center space-x-2">
                        ...
                        <!-- Upload Progress -->
                        <div id="uploadProgress" class="hidden absolute inset-0 bg-gray-100 dark:bg-gray-900 bg-opacity-90 rounded-lg flex items-center justify-center">
                            <div class="text-center">
                                <div class="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500 mb-4"></div>
                                <p class="text-gray-700 dark:text-gray-300">Uploading images...</p>
                                <div class="w-64 bg-gray-100 dark:bg-gray-700 rounded-full h-2 mt-2">
                                    <div id="progressBar" class="bg-blue-500 h-2 rounded-full transition-all duration-300" style="width: 0%"></div>
                                </div>
                                <p id="progressText" class="text-sm text-gray-600 dark:text-gray-400 mt-1">0%</p>
                            </div>
                        </div>
                    </div>
                    @error('images')
                ...
                <div id="filePreview" class="mb-6 hidden">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Selected Images</label>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="bulk_alt_text" id="bulk_alt_text"
                           class="w-full px-4 py-2 bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
    ...
    <div class="mt-8">
        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg">
            <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Recent Uploads</h3>
                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Quick access to your recently uploaded images</p>
            </div>
            <div class="p-6">
                <div id="recentUploads" class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                    <!-- Recent uploads will be loaded here -->
                </div>
                <div id="noRecent" class="text-center py-8 text-gray-600 dark:text-gray-400">
...
function updateFilePreviews() {
    ...
    selectedFiles.forEach((file, index) => {
        ...
        previewDiv.className = 'relative bg-gray-100 dark:bg-gray-700 rounded-lg overflow-hidden border border-gray-300 dark:border-gray-600';

        const reader = new FileReader();
        reader.onload = function(e) {
            previewDiv.innerHTML = `
                <div class="aspect-square bg-gray-200 dark:bg-gray-600 flex items-center justify-center overflow-hidden">
                    <img src="${e.target.result}" alt="${file.name}" class="w-full h-full object-cover">
                </div>
                <div class="p-2">
                    <p class="text-xs text-gray-700 dark:text-gray-300 truncate" title="${file.name}">${file.name}</p>
                    <p class="text-xs text-gray-600 dark:text-gray-500">${formatFileSize(file.size)}</p>
                          <input type="text" name="alt_texts[]" placeholder="Alt text (optional)"
                              class="w-full mt-1 px-2 py-1 bg-gray-200 dark:bg-gray-600 border border-gray-300 dark:border-gray-500 rounded text-xs text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-500">
                </div>
        ...
        .then(data => {
            if (data.length > 0) {
                recentUploads.innerHTML = data.map(image => `
                    <div class="bg-gray-100 dark:bg-gray-700 rounded-lg overflow-hidden border border-gray-300 dark:border-gray-600 hover:border-gray-400 dark:hover:border-gray-500 transition-colors cursor-pointer"
                         onclick="useRecentImage('${image.url}', '${image.filename}')">
                        <div class="aspect-square bg-gray-200 dark:bg-gray-600 flex items-center justify-center overflow-hidden">
                            <img src="${image.url}" alt="${image.filename}" class="w-full h-full object-cover">
                        </div>
                        <div class="p-2">
                            <p class="text-xs text-gray-700 dark:text-gray-300 truncate">${image.filename}</p>
                        </div>
                    </div>
                `).join('');
