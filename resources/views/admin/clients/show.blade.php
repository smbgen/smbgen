...
<div class="px-4 py-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-100 mb-0">{{ $client->name }}</h2>
            <p class="text-gray-400 mt-1">Client Details & History</p>
        </div>
            ...
            <div class="p-6">
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="text-gray-100 font-medium">{{ $client->name }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-400">Email</label>
                        <p class="text-gray-100">{{ $client->email }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-400">Phone</label>
                        <p class="text-gray-100">{{ $client->phone ?? 'Not provided' }}</p>
                    </div>
                    <div>
                    ...
                    @if($client->property_address)
                        <div>
                            <label class="block text-sm font-medium text-gray-400">🏠 Property Address</label>
                            <p class="text-gray-100 whitespace-pre-wrap">{{ $client->property_address }}</p>
                        </div>
                    @endif
                    <div>
                    ...
                    <div>
                        ...
                        <p class="text-gray-100">{{ $client->created_at->format('M j, Y g:i A') }}</p>
                    </div>
                    @if($client->notes)
                        <div>
                            <label class="block text-sm font-medium text-gray-400">Notes</label>
                            <p class="text-gray-100 whitespace-pre-wrap">{{ $client->notes }}</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="p-6">
                <div class="space-y-4">
                    ...
                    @if($client->user_provisioned_at)
                        <div>
                            <label class="block text-sm font-medium text-gray-400">User Provisioned</label>
                            <p class="text-gray-100">{{ $client->user_provisioned_at->format('M j, Y g:i A') }}</p>
                    ...
                    @if($client->account_activated_at)
                        <div>
                            <label class="block text-sm font-medium text-gray-400">Account Activated</label>
                            <p class="text-gray-100">{{ $client->account_activated_at->format('M j, Y g:i A') }}</p>
                    ...
                    @if($client->last_login_at)
                        <div>
                            ...
                            <p class="text-gray-100">{{ $client->last_login_at->format('M j, Y g:i A') }}</p>
                    ...
                    <div class="pt-4 border-t border-gray-600">
                        <label class="block text-sm font-medium text-gray-400 mb-3">Portal Access Control</label>
                ...
                <div class="space-y-4">
                    @foreach($messages as $message)
                        <div class="border border-gray-700 rounded-lg p-4 hover:bg-gray-700 transition-colors">
                            <div class="flex justify-between items-start mb-2">
                                <div class="flex items-center space-x-2">
                                    @if($message->sender_id === $client->user_id)
                                        ...
                                        <span class="text-gray-300">To: {{ $message->recipient->name ?? 'Unknown' }}</span>
                                    @else
                                        <span class="inline-flex px-2 py-1 text-xs font-semibold rounded-full 
                                            {{ $message->is_read ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                            {{ $message->is_read ? 'Read' : 'Unread' }}
                                        </span>
                                        <span class="text-gray-300">From: {{ $message->sender->name ?? 'Unknown' }}</span>
                                    @endif
                                </div>
                            ...
                            <div class="mb-2">
                                <h4 class="font-medium text-gray-100">{{ $message->subject ?? 'No Subject' }}</h4>
                            </div>
