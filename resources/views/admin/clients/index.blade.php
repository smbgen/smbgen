...
<div class="py-6 space-y-6">
    ...
    <div class="admin-card">
        ...
        <form method="GET" action="{{ route('clients.index') }}" class="flex items-end space-x-4">
            <div class="flex-1">
                <label for="search" class="block text-sm font-medium text-gray-300 mb-2">Search clients</label>
                <input 
                    type="text" 
                    id="search" 
                    name="search" 
    ...
    <div class="admin-card border-l-4 border-blue-500">
        <div class="admin-card-header bg-gradient-to-r from-blue-900/30 to-transparent cursor-pointer" onclick="toggleEmailQuickSend()">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <i class="fas fa-envelope-open-text text-blue-400 text-lg"></i>
                    <h3 class="admin-card-title">📧 Quick Email Send</h3>
                    <span class="text-xs text-gray-400">{{ $clients->count() }} {{ $search ? 'filtered' : 'total' }} clients</span>
                </div>
            ...
            <form id="quickEmailForm" method="POST" action="{{ route('admin.email.send') }}" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-2">
                        Recipients
                        <span class="text-blue-400">(check boxes below or use Select button)</span>
                    </label>
                    <div class="flex gap-2">
                        <input 
                            ...
                            value=""
                        >
                        <div class="flex-1 bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 min-h-[38px]">
                ...
                <div>
                    ...
                    <textarea 
                        ...
                        class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent font-mono text-sm"
                    ></textarea>
                </div>
    ...
    <div class="admin-card border-l-4 border-purple-500">
        <div class="admin-card-header bg-gradient-to-r from-purple-900/30 to-transparent">
            <div class="flex items-center gap-3">
                <i class="fas fa-link text-purple-400 text-lg"></i>
                <h3 class="admin-card-title">🔗 Link Google Accounts</h3>
                <span class="text-xs px-2 py-1 bg-purple-600/20 text-purple-300 rounded-full font-medium">{{ $unlinkedGoogleAccounts->count() }} Available</span>
            </div>
        </div>
        <div class="admin-card-body">
            <p class="text-sm text-gray-300 mb-4">Found {{ $unlinkedGoogleAccounts->count() }} Google account(s) that can be linked to client accounts:</p>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach($unlinkedGoogleAccounts as $account)
                <div class="bg-gray-700 rounded-lg p-4 border border-gray-600">
                    <div class="flex items-start justify-between mb-3">
                        <div>
                            <div class="text-sm font-semibold text-white">{{ $account->client->name }}</div>
                            <div class="text-xs text-gray-400 mt-1">{{ $account->email }}</div>
                        </div>
                        <span class="text-xs px-2 py-1 bg-purple-600/20 text-purple-300 rounded">Google ID</span>
                    </div>
                    <div class="bg-gray-800 rounded px-3 py-2 mb-3">
                        <div class="text-xs text-gray-500 mb-1">Google ID:</div>
                        <div class="text-xs font-mono text-gray-200 break-all">{{ $account->google_id }}</div>
                    </div>
    ...
    @else
        <div class="text-center py-12">
            <div class="text-gray-400 mb-6">
                ...
                <p>
                    @if($search ?? false)
                        Try adjusting your search criteria or <a href="{{ route('clients.index') }}" class="text-blue-400 hover:text-blue-300">view all clients</a>.
                    @else
...
function updateRecipientsDisplay() {
    ...
    if (selected.length === 0) {
        display.innerHTML = `<span class="text-sm text-gray-400 flex items-center gap-2">
            <i class="fas fa-info-circle"></i>
            Check boxes in the table below to add recipients
        </span>`;
        return;
    ...
    display.innerHTML = selected.map(client => `
        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-blue-600/20 text-blue-300 rounded-full text-xs border border-blue-500/30">
