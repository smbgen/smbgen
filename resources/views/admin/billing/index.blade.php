...
<div class="min-h-screen">
    ...
    <!-- Debug Panel -->
    @if(config('app.debug'))
    <div class="mb-6 rounded-xl border-2 border-yellow-300 bg-yellow-50 p-6 dark:border-yellow-500 dark:bg-yellow-900/30">
        <h3 class="mb-4 flex items-center gap-2 text-lg font-bold text-yellow-900 dark:text-yellow-300">
            <i class="fas fa-bug"></i>
            Billing Debug Info
        </h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div class="rounded-lg border border-yellow-200 bg-white p-4 dark:border-yellow-700/40 dark:bg-gray-900/50">
                <div class="mb-2 font-semibold text-yellow-900 dark:text-yellow-200">Invoice Stats</div>
                <div class="space-y-1 text-yellow-900 dark:text-yellow-100">
                    <div>Total: <span class="font-mono">{{ \App\Models\Invoice::count() }}</span></div>
                    <div>Paid: <span class="font-mono text-green-700 dark:text-green-400">{{ \App\Models\Invoice::where('status', 'paid')->count() }}</span></div>
                    <div>Unpaid: <span class="font-mono text-red-700 dark:text-red-400">{{ \App\Models\Invoice::where('status', 'unpaid')->count() }}</span></div>
                    <div>Sent: <span class="font-mono text-blue-700 dark:text-blue-400">{{ \App\Models\Invoice::where('status', 'sent')->count() }}</span></div>
                </div>
            </div>
            
            <div class="rounded-lg border border-yellow-200 bg-white p-4 dark:border-yellow-700/40 dark:bg-gray-900/50">
                <div class="mb-2 font-semibold text-yellow-900 dark:text-yellow-200">Revenue Stats</div>
                <div class="space-y-1 text-yellow-900 dark:text-yellow-100">
                    @php
                    ...
                    @endphp
                    <div>Paid: <span class="font-mono text-green-700 dark:text-green-400">${{ number_format($totalRevenue / 100, 2) }}</span></div>
                    <div>Pending: <span class="font-mono text-yellow-700 dark:text-yellow-400">${{ number_format($pendingRevenue / 100, 2) }}</span></div>
        ...
        <div class="mt-4 border-t border-yellow-300 pt-4 dark:border-yellow-700">
            <div class="flex items-center gap-4">
                <span class="text-xs text-yellow-800 dark:text-yellow-300">Debug mode enabled in .env</span>
            </div>
        </div>
    </div>
    @endif
        ...
        <div class="bg-white dark:bg-gray-800/50 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
            ...
            @if($stripeStatus['success'])
                <div class="space-y-3">
                    ...
                    <div class="flex items-center justify-between text-sm">
                        <span class="text-gray-600 dark:text-gray-400">Invoices with Payment Links</span>
                        <span class="text-gray-900 dark:text-gray-200 font-medium">{{ \App\Models\Invoice::whereNotNull('stripe_payment_url')->count() }}</span>
                    </div>
                    <div class="flex items-center justify-between text-sm">
                        <span class="text-gray-600 dark:text-gray-400">Stripe Customers</span>
                        <span class="text-gray-900 dark:text-gray-200 font-medium">{{ \App\Models\Client::whereNotNull('stripe_customer_id')->count() }}</span>
                    </div>
            ...
            @else
                ...
                <div class="bg-gray-100 dark:bg-gray-900/50 rounded-lg p-3 text-xs font-mono text-gray-600 dark:text-gray-500">
    ...
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100">All Invoices</h2>
    ...
    <div class="overflow-x-auto rounded-lg border border-gray-300 dark:border-gray-800 bg-white dark:bg-gray-900/50">
        <table class="min-w-full divide-y divide-gray-300 dark:divide-gray-800">
            <thead class="bg-gray-100 dark:bg-gray-800/60">
                <tr>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-700 dark:text-gray-400">#</th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-700 dark:text-gray-400">Client</th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-700 dark:text-gray-400">Date</th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-700 dark:text-gray-400">Status</th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-700 dark:text-gray-400">Payment</th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-700 dark:text-gray-400">Total</th>
                    <th scope="col" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-700 dark:text-gray-400">Actions</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-300 dark:divide-gray-800">
                @foreach($invoices as $invoice)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/40">
                        <td class="px-4 py-3 text-gray-600 dark:text-gray-300">{{ $invoice->id }}</td>
                        <td class="px-4 py-3">
                            <a href="{{ route('admin.billing.show', $invoice->user) }}" class="text-blue-400 hover:text-blue-300">
                                {{ $invoice->user->name }}
                            </a>
                        </td>
                        <td class="px-4 py-3 text-gray-600 dark:text-gray-300">{{ $invoice->created_at->format('Y-m-d') }}</td>
                        <td class="px-4 py-3">
                            @php
                                $badge = match($invoice->status) {
                                    'paid' => 'bg-green-900/40 text-green-300 ring-1 ring-green-800',
                                    'sent' => 'bg-blue-900/40 text-blue-300 ring-1 ring-blue-800',
                                    default => 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 ring-1 ring-gray-300 dark:ring-gray-700',
                                };
                            @endphp
                        ...
                        <td class="px-4 py-3">
                            <div class="flex flex-col gap-1">
                                ...
                                @else
                                    <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 ring-1 ring-gray-300 dark:ring-gray-700">
                        ...
                        <td class="px-4 py-3 text-gray-900 dark:text-gray-100">${{ number_format($invoice->total_amount / 100, 2) }}</td>
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                ...
                                <!-- Copy Payment Link -->
                                @if($invoice->stripe_payment_url)
                                    <button onclick="copyToClipboard('{{ $invoice->stripe_payment_url }}')" class="inline-flex items-center gap-1 px-2 py-1 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 rounded text-xs transition-colors" title="Copy Payment Link">
    ...
    @else
        <div class="text-center text-gray-600 dark:text-gray-400 py-8">No invoices found.</div>
    @endif
...
<div id="payment-url-modal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 max-w-lg w-full mx-4 border border-gray-200 dark:border-gray-700 shadow-2xl">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-xl font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                <i class="fas fa-check-circle text-green-400"></i>
                Payment Link Generated
            </h3>
            <button onclick="closeModal()" class="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">
                <i class="fas fa-times"></i>
            </button>
        </div>
        
        <p class="text-gray-700 dark:text-gray-300 mb-4">Share this link with your client to collect payment:</p>
        
        <div class="bg-gray-100 dark:bg-gray-900 rounded-lg p-4 mb-4 flex items-center gap-3">
        ...
        <div class="flex gap-3">
            ...
            <button onclick="closeModal()" class="flex-1 px-4 py-2 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-900 dark:text-white rounded-lg text-sm font-medium transition-colors">
                Close
            </button>
        </div>
    </div>
