...
<div class="min-h-screen">
    ...
    <div class="space-y-4">
        @forelse($invoices as $invoice)
            <div class="bg-white dark:bg-gray-800/50 rounded-xl border border-gray-300 dark:border-gray-700 hover:border-gray-400 dark:hover:border-gray-600 transition-all shadow-lg hover:shadow-xl">
                <div class="p-6">
                    <div class="flex items-start justify-between mb-4">
                        <div class="flex-1">
                            <div class="flex items-center gap-4 mb-2">
                                <h3 class="text-2xl font-bold text-gray-900 dark:text-white">Invoice #{{ $invoice->id }}</h3>
                                @php
                        ...
                        <div class="text-right">
                            <div class="text-3xl font-bold text-gray-900 dark:text-white">${{ number_format($invoice->total_amount / 100, 2) }}</div>
                            <div class="text-sm text-gray-600 dark:text-gray-400">Total Amount</div>
                        </div>
                    </div>

                    @if($invoice->memo)
                        <div class="mb-4 p-4 bg-gray-100 dark:bg-gray-900/50 rounded-lg border border-gray-300 dark:border-gray-700">
                            <p class="text-gray-700 dark:text-gray-300 text-sm"><i class="fas fa-comment-alt mr-2 text-gray-600 dark:text-gray-500"></i>{{ $invoice->memo }}</p>
                        </div>
                    @endif
                    ...
                    @if($invoice->items && $invoice->items->count() > 0)
                        <div class="mb-4">
                            <h4 class="text-sm font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider mb-2">Items</h4>
                            <div class="space-y-2">
                                @foreach($invoice->items as $item)
                                    <div class="flex items-center justify-between p-3 bg-gray-100 dark:bg-gray-900/30 rounded-lg">
                                        <div class="flex-1">
                                            <div class="text-gray-900 dark:text-white font-medium">{{ $item->description }}</div>
                                            <div class="text-sm text-gray-600 dark:text-gray-400">Qty: {{ $item->quantity }} × ${{ number_format($item->unit_amount / 100, 2) }}</div>
                                        </div>
                                        <div class="text-gray-900 dark:text-white font-semibold">${{ number_format($item->total_amount / 100, 2) }}</div>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="flex items-center gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                        @if($invoice->status !== 'paid')
                            ...
                            @if($invoice->hasStripePaymentUrl())
                                ...
                                <button onclick="copyToClipboard('{{ $invoice->stripe_payment_url }}')" class="px-4 py-3 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-900 dark:text-white rounded-lg transition-colors font-medium" title="Copy Payment Link">
        ...
        @empty
            <div class="bg-white dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 p-12 text-center">
                <i class="fas fa-file-invoice text-gray-600 text-5xl mb-4"></i>
                <h3 class="text-xl font-semibold text-gray-700 dark:text-gray-400 mb-2">No Invoices Yet</h3>
                <p class="text-gray-600 dark:text-gray-500 mb-6">Create your first invoice for this client</p>
...
<div id="refundModal" class="hidden fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full mx-4 border border-gray-200 dark:border-gray-700">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <h3 class="text-xl font-bold text-gray-900 dark:text-white">Refund Payment</h3>
        </div>
        <form id="refundForm" method="POST" action="">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input 
                            ...
                            step="0.01" 
                            min="0.01"
                            class="w-full pl-8 pr-4 py-3 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:ring-2 focus:ring-red-500 focus:border-red-500"
                            placeholder="Full refund"
                        >
                    </div>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-2">Leave blank for full refund</p>
                </div>
                <div>
                    ...
                    <textarea 
                        name="reason" 
                        rows="3"
                        class="w-full px-4 py-3 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white focus:ring-2 focus:ring-red-500 focus:border-red-500 resize-none"
            ...
            <div class="p-6 border-t border-gray-200 dark:border-gray-700 flex gap-3">
                <button 
                    ...
                    class="flex-1 px-4 py-3 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-900 dark:text-white rounded-lg transition-colors font-medium"
                >
                    Cancel
                </button>
                <button 
