...
<div class="px-4 sm:px-6 lg:px-8 py-6">
    <!-- Flash Messages -->
    @if (session('status'))
        <div class="bg-green-900/20 border border-green-500 text-green-300 px-6 py-4 rounded-lg mb-8 flex items-center">
    ...
    @if (session('error'))
        <div class="bg-red-900/20 border border-red-500 text-red-300 px-6 py-4 rounded-lg mb-8 flex items-center">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        @forelse ($users as $user)
            ...
            @endphp
            <div class="bg-gray-800 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border {{ $isFullyConnected ? 'border-green-500' : ($isPartiallyConnected ? 'border-yellow-500' : 'border-gray-700') }}">
                <!-- User Header with Connection Status -->
                <div class="p-6 {{ $isFullyConnected ? 'bg-gradient-to-r from-green-900/30 to-blue-900/30' : ($isPartiallyConnected ? 'bg-gradient-to-r from-yellow-900/30 to-orange-900/30' : 'bg-gray-900/50') }} border-b border-gray-700">
                    <div class="flex items-start justify-between">
                        <div class="flex items-center space-x-4 flex-1">
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-gray-400 flex items-center text-sm mt-1">
                                ...
                                <div class="flex items-center mt-2 gap-2 flex-wrap">
                                    ...
                                    @else
                                        <span class="inline-flex items-center px-2.5 py-1 text-xs font-semibold rounded-full bg-gray-600 text-gray-200">
                                            <i class="fas fa-user mr-1"></i>User
                                        </span>
                                    @endif
                                    @if($user->email_verified_at)
                                        <span class="inline-flex items-center px-2.5 py-1 text-xs font-semibold rounded-full bg-green-900/30 text-green-300">
                        ...
                        <div class="flex flex-col items-end gap-2">
                            @if($isFullyConnected)
                                ...
                                <div class="text-xs text-gray-400">OAuth + Calendar</div>
                            @elseif($hasGoogleOAuth && !$hasGoogleCalendar)
                                ...
                                <div class="text-xs text-gray-400">No Calendar</div>
                            @elseif($hasGoogleCalendar && !$hasGoogleOAuth)
                                ...
                                <div class="text-xs text-gray-400">Password Login</div>
                            @else
                                ...
                                <div class="text-xs text-gray-400">No Google</div>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6">
                    ...
                    @if ($user->google_id)
                        <div class="mb-4">
                            ...
                            <div class="bg-gradient-to-r from-red-900/20 to-orange-900/20 rounded-lg p-4 border border-red-500/30">
                                <div class="space-y-3">
                                    <div class="flex items-center justify-between">
                                        <span class="text-xs text-gray-400 uppercase tracking-wider">Login Type</span>
                                    ...
                                    <div class="flex items-center justify-between">
                                        <span class="text-xs text-gray-400 uppercase tracking-wider">Google ID</span>
                                        <code class="text-xs text-red-300 bg-gray-900 px-3 py-1 rounded font-mono">{{ $user->google_id }}</code>
                                    </div>
                                    <div class="flex items-center justify-between">
                                        <span class="text-xs text-gray-400 uppercase tracking-wider">Authentication</span>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="bg-gradient-to-r from-blue-900/20 to-purple-900/20 rounded-lg p-4 border border-blue-500/30">
                                <div class="space-y-3">
                                    ...
                                    <div class="flex items-center justify-between">
                                        <span class="text-xs text-gray-400 uppercase tracking-wider">Google Account</span>
                                        <code class="text-xs text-blue-300 bg-gray-900 px-3 py-1 rounded font-mono">{{ $googleCred->external_account_email }}</code>
                                    </div>
                                    @endif
                                    
                                    @if($googleCred->calendar_id)
                                    <div class="flex items-center justify-between">
                                        <span class="text-xs text-gray-400 uppercase tracking-wider">Calendar ID</span>
                                        <code class="text-xs text-green-300 bg-gray-900 px-3 py-1 rounded font-mono">{{ $googleCred->calendar_id }}</code>
                                    </div>
                                    @endif
                                    ...
                                    <div class="flex items-center justify-between">
                                        <span class="text-xs text-gray-400 uppercase tracking-wider">Connected</span>
                                        <span class="text-xs text-gray-300">{{ $googleCred->created_at->diffForHumans() }}</span>
                                    </div>
                                    @endif
                                </div>
                            </div>
                    ...
                    @if (!$user->google_id && !$hasGoogleCalendar)
                        <div class="mb-4">
                            <div class="bg-gray-900/50 rounded-lg p-6 border-2 border-dashed border-gray-700 text-center">
                                <div class="mb-3">
                                    <i class="fab fa-google text-gray-600 text-4xl"></i>
                                </div>
                                <p class="text-gray-400 font-medium mb-1">No Google Connection</p>
                                <p class="text-gray-500 text-sm">This user has no Google OAuth login and no Google Calendar integration</p>
                            </div>
                        </div>
                    @elseif (!$hasGoogleCalendar && $user->google_id)
                        <div class="mb-4">
                            <div class="bg-yellow-900/20 rounded-lg p-4 border border-yellow-500/30 text-center">
                                ...
                                <p class="text-yellow-300 text-sm font-medium">Google Calendar Not Connected</p>
                                <p class="text-gray-400 text-xs mt-1">User has Google OAuth login but no calendar integration</p>
                            </div>
                        </div>
                    @elseif ($hasGoogleCalendar && !$user->google_id)
                        <div class="mb-4">
                            <div class="bg-blue-900/20 rounded-lg p-4 border border-blue-500/30 text-center">
                                ...
                                <p class="text-blue-300 text-sm font-medium">Password-Based Login</p>
                                <p class="text-gray-400 text-xs mt-1">User has calendar integration but uses traditional password login</p>
                            </div>
                        </div>
                    @endif
        ...
        @empty
            <div class="col-span-full">
                <div class="text-center py-20 bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl border-2 border-dashed border-gray-700">
                    <div class="mb-6">
                        <i class="fab fa-google text-gray-600 text-6xl"></i>
                    </div>
                    <h3 class="text-2xl font-bold text-gray-300 mb-2">No Users Found</h3>
                    <p class="text-gray-400 mb-6">There are no users in the system yet to monitor OAuth connections.</p>
                    ...
                    <table class="min-w-full">
                        <thead>
                            <tr class="border-b border-gray-700">
                                <th class="text-left py-3 px-4 font-medium text-gray-300 text-sm">User</th>
                                <th class="text-left py-3 px-4 font-medium text-gray-300 text-sm">Email</th>
                                <th class="text-left py-3 px-4 font-medium text-gray-300 text-sm">Provider</th>
                                <th class="text-left py-3 px-4 font-medium text-gray-300 text-sm">Google ID</th>
                                <th class="text-left py-3 px-4 font-medium text-gray-300 text-sm">IP Address</th>
                                <th class="text-left py-3 px-4 font-medium text-gray-300 text-sm">Status</th>
                                <th class="text-left py-3 px-4 font-medium text-gray-300 text-sm">When</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-700">
                            @foreach ($recentLoginAttempts as $attempt)
                                <tr class="hover:bg-gray-700/50 transition-colors">
                                    <td class="py-4 px-4">
                                        <div class="flex items-center space-x-3">
                                            @if(optional($attempt->user)->name)
                                                ...
                                                <span class="text-gray-100 font-medium">{{ optional($attempt->user)->name ?? 'Unknown' }}</span>
                                            @else
                                    ...
                                    <td class="py-4 px-4">
                                        <span class="text-gray-300">{{ $attempt->email ?? '—' }}</span>
                                    </td>
                                    <td class="py-4 px-4">
                                        <div class="flex items-center space-x-2">
                                            ...
                                            <span class="text-gray-300">{{ ucfirst($attempt->provider) }}</span>
                                        </div>
                                    </td>
                                    <td class="py-4 px-4">
                                        <code class="bg-gray-700 text-gray-300 px-2 py-1 rounded text-xs">{{ $attempt->provider_id }}</code>
                                    </td>
                                    <td class="py-4 px-4">
                                        <div class="flex items-center space-x-2">
                                            <i class="fas fa-globe text-gray-500 text-sm"></i>
                                            <span class="text-gray-300 font-mono text-sm">{{ $attempt->ip_address ?? 'N/A' }}</span>
                                        </div>
                                    </td>
                                    <td class="py-4 px-4">
