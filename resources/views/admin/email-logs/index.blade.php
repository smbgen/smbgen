    ...
    <div class="admin-page-header">
        ...
        <div class="flex gap-2 flex-wrap">
            ...
            <div class="flex items-center gap-2 bg-gray-800 px-3 py-2 rounded-lg">
                ...
                <select id="autoRefreshInterval" class="bg-gray-700 border-gray-600 text-white text-sm rounded px-2 py-1" disabled>
            ...
            <div class="relative">
                ...
                <div id="bulkActionsMenu" class="hidden absolute right-0 mt-2 w-48 bg-gray-800 rounded-lg shadow-xl border border-gray-700 z-10">
                    <button onclick="bulkResend()" class="w-full text-left px-4 py-2 hover:bg-gray-700 text-gray-200 rounded-t-lg">
                        <i class="fas fa-redo mr-2"></i>Resend Failed
                    </button>
                    <button onclick="bulkDelete()" class="w-full text-left px-4 py-2 hover:bg-gray-700 text-gray-200">
                        <i class="fas fa-trash mr-2"></i>Delete Selected
                    </button>
                    <button onclick="clearOldLogs()" class="w-full text-left px-4 py-2 hover:bg-gray-700 text-gray-200 rounded-b-lg">
    ...
    <div class="admin-card border border-purple-500/30 mt-6">
        <div class="admin-card-header cursor-pointer" onclick="toggleDebugPanel()">
            ...
            <div class="flex items-center gap-3">
                ...
                <span class="text-xs px-2 py-1 bg-purple-600/20 text-purple-300 rounded">DEV</span>
            </div>
        ...
        <div id="debugPanelContent" class="hidden admin-card-body space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="bg-gray-800 rounded-lg p-4">
                    ...
                    <div class="text-sm font-mono text-green-400">{{ app()->environment() }}</div>
                </div>
                <div class="bg-gray-800 rounded-lg p-4">
                    <div class="text-xs text-gray-400 mb-1">Mailer</div>
                    <div class="text-sm font-mono text-blue-400">{{ config('mail.default') }}</div>
                </div>
                <div class="bg-gray-800 rounded-lg p-4">
                    <div class="text-xs text-gray-400 mb-1">From Address</div>
                    <div class="text-sm font-mono text-purple-400">{{ config('mail.from.address') }}</div>
                </div>
            </div>
            ...
            <div class="bg-gray-800 rounded-lg p-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                        <div class="text-xs text-gray-400">Open Tracking URL</div>
                        <div class="text-xs font-mono text-gray-300 bg-gray-900 px-2 py-1 rounded mt-1 break-all">
                            {{ route('email.track.open', ['id' => 'TRACKING_ID']) }}
                        </div>
                    </div>
                    <div>
                        <div class="text-xs text-gray-400">Click Tracking URL</div>
                        <div class="text-xs font-mono text-gray-300 bg-gray-900 px-2 py-1 rounded mt-1 break-all">
            ...
            <div class="bg-gray-800 rounded-lg p-4">
                ...
                <div class="space-y-2">
                    <div class="flex items-center gap-2 text-xs">
                        ...
                        <span class="text-gray-300 font-mono">MessageSent → LogSentEmail</span>
                        <span class="text-gray-500">(Logs all emails)</span>
                    </div>
                    <div class="flex items-center gap-2 text-xs">
                        <i class="fas fa-check-circle text-green-400"></i>
                        <span class="text-gray-300 font-mono">MessageSending → Nightwatch</span>
            ...
            <div class="bg-gray-800 rounded-lg p-4">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div class="text-center">
                        ...
                        <div class="text-xs text-gray-400">With Tracking</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-yellow-400">{{ $withOpens }}</div>
                        <div class="text-xs text-gray-400">With Opens</div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gray-800 rounded-lg p-4">
                ...
                <div class="space-y-2">
                    ...
                    @forelse($recentLogs as $log)
                        <div class="flex items-center justify-between text-xs bg-gray-900 rounded px-3 py-2">
                            <div class="flex items-center gap-3 flex-1 min-w-0">
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ 
                                    $log->status === 'opened' || $log->status === 'clicked' ? 'bg-green-900 text-green-300' :
                                    ($log->status === 'sent' || $log->status === 'delivered' ? 'bg-blue-900 text-blue-300' :
                                    ($log->status === 'failed' || $log->status === 'bounced' ? 'bg-red-900 text-red-300' : 'bg-gray-700 text-gray-300'))
                                }}">
                                    {{ $log->status }}
                                </span>
                                <span class="text-gray-300 truncate flex-1">{{ $log->to_email }}</span>
                ...
                <div id="advancedFilters" class="hidden">
                    <div class="border-t border-gray-700 pt-4 mt-2">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <!-- Engagement Filter -->
                            <div>
                                <label class="block text-sm font-medium text-gray-300 mb-2">Engagement</label>
                            ...
                            <!-- Has Booking Filter -->
                            <div>
                                <label class="block text-sm font-medium text-gray-300 mb-2">Booking</label>
                            ...
                            <!-- Sent By User -->
                            <div>
                                <label class="block text-sm font-medium text-gray-300 mb-2">Sent By</label>
            ...
            <table class="min-w-full divide-y divide-gray-700">
                <thead class="bg-gray-800">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Engagement</th>
                ...
                <tbody class="bg-gray-900 divide-y divide-gray-700">
                    @forelse($emailLogs as $log)
                        <tr class="hover:bg-gray-800 transition-colors cursor-pointer" onclick="handleRowClick(event, '{{ route('admin.email-logs.show', $log->id) }}')"  data-log-id="{{ $log->id }}">
                            <!-- Checkbox -->
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ 
                                    $log->status === 'opened' ? 'bg-green-900/30 text-green-300 border border-green-700' :
                                    ($log->status === 'clicked' ? 'bg-purple-900/30 text-purple-300 border border-purple-700' :
                                    ($log->status === 'delivered' || $log->status === 'sent' ? 'bg-blue-900/30 text-blue-300 border border-blue-700' :
                                    ($log->status === 'bounced' || $log->status === 'failed' ? 'bg-red-900/30 text-red-300 border border-red-700' :
                            ...
                            <!-- To Email -->
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-100">{{ $log->to_email }}</div>
                                @if($log->user && $log->user->name)
                                    <div class="text-xs text-gray-400">Sent by: {{ $log->user->name }}</div>
                                @endif
                            </td>

                            <!-- Subject -->
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-100 max-w-xs truncate" title="{{ $log->subject }}">
                                    {{ $log->subject }}
                                </div>
...
<div id="emailPreviewModal" class="hidden fixed inset-0 bg-black bg-opacity-75 z-50 flex items-center justify-center p-4">
    <div class="bg-gray-900 rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
        <div class="bg-gray-800 px-6 py-4 border-b border-gray-700 flex items-center justify-between">
            <h3 class="text-lg font-semibold text-gray-100" id="modalSubject">Email Preview</h3>
            <button onclick="closeEmailPreview()" class="text-gray-400 hover:text-gray-200">
