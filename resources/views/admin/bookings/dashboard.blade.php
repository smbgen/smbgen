...
<div class="min-h-screen">
    ...
    <div id="bookings" class="bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 border border-gray-200 dark:border-gray-700 shadow-xl">
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
                <div class="bg-gray-300 dark:bg-gray-600 rounded-xl p-2">
                    <i class="fas fa-list text-gray-900 dark:text-white"></i>
                </div>
                All Bookings
            </h2>
            @if(Route::has('booking.wizard'))
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-gray-300 dark:border-gray-700">
                        ...
                        <th class="text-left py-4 px-4 text-gray-600 dark:text-gray-400 font-semibold text-sm uppercase tracking-wider">Staff</th>
                        <th class="text-left py-4 px-4 text-gray-600 dark:text-gray-400 font-semibold text-sm uppercase tracking-wider">Property</th>
                        <th class="text-left py-4 px-4 text-gray-600 dark:text-gray-400 font-semibold text-sm uppercase tracking-wider">Calendar</th>
                ...
                <tbody class="divide-y divide-gray-300 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-100 dark:hover:bg-gray-700/30 transition-colors cursor-pointer" onclick="window.location='{{ route('admin.bookings.show', $booking) }}'">
                        <td class="py-4 px-4">
                        ...
                        <td class="py-4 px-4">
                            ...
                            <div class="text-gray-500 dark:text-gray-500 text-xs mt-1">{{ $booking->duration }} min</div>
                        </td>
                        <td class="py-4 px-4">
                            @if($booking->staff)
                                <div class="text-gray-900 dark:text-white text-sm font-medium">{{ $booking->staff->name }}</div>
                                <div class="text-gray-600 dark:text-gray-400 text-xs">{{ $booking->staff->email }}</div>
                            @else
                        ...
                        <td class="py-4 px-4">
                            @if($booking->property_address)
                                <div class="text-gray-700 dark:text-gray-300 text-sm max-w-xs" title="{{ $booking->property_address }}">
                                    <i class="fas fa-map-marker-alt text-xs mr-1 text-gray-500 dark:text-gray-400"></i>
                            ...
                            @if($booking->google_calendar_event_id)
                                <div class="flex flex-col gap-1">
                                    ...
                                    @if($booking->google_meet_link)
                                        <a href="{{ $booking->google_meet_link }}" target="_blank" class="flex items-center gap-1.5 text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-100 text-xs transition-colors">
                        ...
                        <td class="py-4 px-4 text-right" onclick="event.stopPropagation()">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form action="{{ route('admin.bookings.send-reminder', $booking) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="bg-gray-600 hover:bg-gray-500 text-white rounded-lg px-3 py-2 text-sm transition-colors flex items-center gap-1.5" title="Send Reminder">
                                ...
                                <form action="{{ route('admin.bookings.destroy', $booking) }}" method="POST" class="inline" onsubmit="return confirm('Delete this booking?');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="bg-red-600 hover:bg-red-700 text-white rounded-lg px-3 py-2 text-sm transition-colors flex items-center gap-1.5" title="Delete">
        ...
        <div class="mt-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="text-sm text-gray-600 dark:text-gray-400">
                Showing <span class="text-gray-900 dark:text-white font-semibold">{{ $bookings->firstItem() }}</span>
                -
                <span class="text-gray-900 dark:text-white font-semibold">{{ $bookings->lastItem() }}</span>
                of
                <span class="text-gray-900 dark:text-white font-semibold">{{ $bookings->total() }}</span>
                bookings
            </div>
            <div class="bg-gray-100 dark:bg-gray-900/50 rounded-xl p-2 border border-gray-300 dark:border-gray-700">
                {{ $bookings->withQueryString()->links() }}
            </div>
        </div>
        @else
        <div class="text-center py-12">
            <i class="fas fa-calendar-plus text-gray-400 dark:text-gray-600 text-6xl mb-4"></i>
            <p class="text-gray-700 dark:text-gray-400 text-lg mb-4">No bookings yet</p>
    ...
    <div class="mt-8 bg-white dark:bg-gray-800/50 backdrop-blur-sm rounded-2xl border border-gray-200 dark:border-gray-700 shadow-xl overflow-hidden">
        <details class="group">
            <summary class="cursor-pointer p-6 flex items-center justify-between hover:bg-gray-100 dark:hover:bg-gray-700/30 transition-colors">
                <div class="flex items-center gap-3">
                    <div class="bg-yellow-500/20 rounded-xl p-2">
                        <i class="fas fa-exclamation-triangle text-yellow-400"></i>
                    </div>
                    <div>
                        <h3 class="text-lg font-bold text-gray-900 dark:text-white">Google Calendar Connection Issues?</h3>
                        <p class="text-gray-600 dark:text-gray-400 text-sm">Click to view troubleshooting steps</p>
                    </div>
                </div>
                <i class="fas fa-chevron-down text-gray-600 dark:text-gray-400 transition-transform group-open:rotate-180"></i>
            </summary>
                ...
                <div class="mt-4 space-y-4">
                    ...
                    <div class="bg-gray-100 dark:bg-gray-700/30 rounded-xl p-4">
                        ...
                        <ol class="space-y-3 text-gray-700 dark:text-gray-300">
                            <li class="flex gap-3">
                                <span class="flex-shrink-0 bg-gray-400 dark:bg-gray-600 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm font-bold">1</span>
                                <div>
                                    <p class="font-medium text-gray-900 dark:text-white">Open Google Cloud Console</p>
                                    <a href="https://console.cloud.google.com/apis/credentials" target="_blank" class="text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-100 text-sm inline-flex items-center gap-1 mt-1">
                            ...
                            <li class="flex gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="mt-2 space-y-2">
                                        <div class="bg-gray-100 dark:bg-gray-900/50 rounded-lg p-3 border border-gray-300 dark:border-gray-600">
                                            <code class="text-green-600 dark:text-green-400 text-sm">https://{{ request()->getHost() }}/auth/google/callback</code>
                                            <button onclick="navigator.clipboard.writeText('https://{{ request()->getHost() }}/auth/google/callback')" class="ml-2 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white" title="Copy to clipboard">
                                                <i class="fas fa-copy text-xs"></i>
                                            </button>
                                        </div>
                                        <div class="bg-gray-100 dark:bg-gray-900/50 rounded-lg p-3 border border-gray-300 dark:border-gray-600">
                                            <code class="text-green-600 dark:text-green-400 text-sm">https://{{ request()->getHost() }}/admin/calendar/callback</code>
                                            <button onclick="navigator.clipboard.writeText('https://{{ request()->getHost() }}/admin/calendar/callback')" class="ml-2 text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white" title="Copy to clipboard">
                                                <i class="fas fa-copy text-xs"></i>
                                            </button>
                                        </div>
                                    </div>
                            ...
                            <li class="flex gap-3">
                                <span class="flex-shrink-0 bg-gray-400 dark:bg-gray-600 text-white rounded-full w-6 h-6 flex items-center justify-center text-sm font-bold">4</span>
                                <div>
                                    <p class="font-medium text-gray-900 dark:text-white">Save Changes</p>
                                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Click "SAVE" at the bottom. Changes take effect immediately.</p>
                                </div>
                            </li>
                            <li class="flex gap-3">
                                ...
                                <div>
                                    <p class="font-medium text-gray-900 dark:text-white">Test the Connection</p>
                                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Return to this page and try connecting your calendar again.</p>
                                </div>
                            </li>
                        </ol>
                    </div>
                    ...
                    <div class="flex items-center gap-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                        <a href="{{ asset('app/docs/GOOGLE_OAUTH_SETUP.md') }}" target="_blank" class="text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-100 text-sm inline-flex items-center gap-2">
                            <i class="fas fa-book"></i>
                            Full Documentation
                        </a>
                        <a href="https://console.cloud.google.com/apis/credentials" target="_blank" class="text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-100 text-sm inline-flex items-center gap-2">
