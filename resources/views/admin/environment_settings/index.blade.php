...
<div class="max-w-6xl mx-auto py-6">
    ...
    <div class="mb-6 flex gap-4 text-xs">
        <div class="flex items-center gap-2">
            ...
            <span class="text-gray-400">Set via .env (read-only)</span>
        </div>
        <div class="flex items-center gap-2">
            <div class="w-3 h-3 bg-gray-600 rounded"></div>
            <span class="text-gray-400">Editable (no .env override)</span>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.environment_settings.update') }}" class="space-y-8">
                    ...
                    @foreach($category['items'] as $key => $setting)
                        ...
                        <div class="form-group">
                            <label for="{{ $categoryKey }}_{{ $key }}" class="form-label flex items-center gap-2">
                                {{ $setting['label'] }}
                                @if($isEnvControlled)
                                    <span class="text-xs px-2 py-0.5 bg-blue-900/50 text-blue-300 rounded border border-blue-800">
                                        .env: {{ $setting['env_key'] }}
                                    </span>
                                @endif
                            </label>
                            ...
                            @if($setting['type'] === 'boolean')
                                <label class="flex items-center gap-3 p-3 bg-gray-700/30 rounded-lg border border-gray-600 cursor-pointer hover:bg-gray-700/50 transition-colors">
                                    <input 
                                        type="checkbox" 
                                    ...
                                    >
                                    <div>
                                        <span class="text-sm font-medium text-gray-200">Enable {{ $setting['label'] }}</span>
                            ...
                            @else
                                <input 
                                    ...
                                    value="{{ $displayValue }}" 
                                    {{ $isReadonly ? 'readonly' : '' }}
                                    class="form-input {{ $isReadonly ? 'cursor-not-allowed bg-gray-700/30 text-gray-400' : '' }}"
                                >
                                @if(isset($setting['description']))
                                    <p class="form-help text-gray-400">
                                        {{ $setting['description'] }}
                                    </p>
                                @endif
                            @endif
                            ...
                            @if($isEnvControlled)
                                <p class="form-help text-blue-400 text-xs mt-1">
                                    🔒 Controlled by .env file - cannot be changed via UI
                                </p>
                            @elseif($setting['readonly'] ?? false)
                                <p class="form-help text-gray-500 text-xs mt-1">
        ...
        <div class="alert alert-warning">
            <div class="flex items-start">
                ...
                <div>
                    <h4 class="text-sm font-medium text-yellow-300 mb-1">How Environment Settings Work</h4>
                    <ul class="text-sm text-yellow-200/80 space-y-1 list-disc list-inside">
                        <li>Settings with values in the <code class="bg-gray-700 px-1 rounded">.env</code> file <strong>always take precedence</strong> and cannot be changed here.</li>
                        <li>Settings without .env values can be configured in the database via this form.</li>
                        <li>To make a setting editable, remove it from your <code class="bg-gray-700 px-1 rounded">.env</code> file.</li>
                        <li>To lock a setting, add it to your <code class="bg-gray-700 px-1 rounded">.env</code> file with your desired value.</li>
                    </ul>
                </div>
            </div>
        </div>
