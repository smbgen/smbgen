...
<div class="min-h-screen">
    ...
    <!-- Flash Messages -->
    @if(session('success'))
        <div class="mb-6 p-4 rounded-xl bg-green-500/20 border-2 border-green-500 text-green-100 backdrop-blur-sm">
...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-yellow-900/30 border-2 border-yellow-500 rounded-lg p-4 mb-6">
        <h3 class="text-yellow-300 font-bold text-lg mb-3">🐛 DEBUG INFO</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
                <div class="text-yellow-200 font-semibold mb-2">Current User:</div>
                <div class="bg-gray-900/50 rounded p-3 space-y-1 font-mono text-xs">
                    <div class="text-gray-300">ID: <span class="text-white">{{ auth()->user()->id }}</span></div>
                    <div class="text-gray-300">Name: <span class="text-white">{{ auth()->user()->name }}</span></div>
                    <div class="text-gray-300">Email: <span class="text-white">{{ auth()->user()->email }}</span></div>
                    <div class="text-gray-300">Role: <span class="text-white">{{ auth()->user()->role }}</span></div>
                </div>
            </div>
            <div>
                <div class="text-yellow-200 font-semibold mb-2">Google Calendar Status:</div>
                <div class="bg-gray-900/50 rounded p-3 space-y-1 font-mono text-xs">
                    <div class="text-gray-300">Connection: 
                    ...
                    <div class="text-gray-300">Calendar ID: <span class="text-white">{{ auth()->user()->googleCredential->calendar_id ?? 'none' }}</span></div>
                    <div class="text-gray-300">Availabilities: <span class="text-white">{{ $availabilities->count() }}</span></div>
                </div>
            </div>
        </div>
        @if(!auth()->user()->googleCredential)
            <div class="mt-3 bg-red-900/30 border border-red-500 rounded p-3">
                <p class="text-red-300 font-semibold">⚠️ Google Calendar NOT connected!</p>
                <p class="text-red-200 text-sm mt-1">You won't appear in the booking form until you connect Google Calendar.</p>
        ...
        @endif
        <div class="mt-3 text-yellow-200 text-xs">
    ...
    <div class="bg-gray-800 rounded-lg shadow-lg p-4 mb-6">
        <form method="GET" action="{{ route('admin.availability.index') }}" class="flex items-center gap-4">
            <label class="text-gray-300 font-medium">Managing availability for:</label>
            <select name="user_id" onchange="this.form.submit()" class="bg-gray-700 border border-gray-600 text-white rounded-lg px-3 py-2 focus:ring-blue-500 focus:border-blue-500">
    ...
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-white">
            ...
            @if(count($staffMembers) > 1)
                <span class="text-gray-400 text-lg">- {{ $selectedUser->name }}</span>
            @endif
        </h2>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-700">
                    <thead class="bg-gray-900">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Day</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">Start Time</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase tracking-wider">End Time</th>
                        ...
                        @foreach($availabilities as $availability)
                            <tr class="hover:bg-gray-700 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="font-medium text-white">{{ $availability->day_name }}</span>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <a href="{{ route('admin.availability.edit', $availability) }}" class="text-blue-400 hover:text-blue-300 mr-3">Edit</a>
                                    <form action="{{ route('admin.availability.destroy', $availability) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-400 hover:text-red-300" onclick="return confirm('Are you sure you want to delete this availability?')">
                                            Delete
                                        </button>
                                    </form>
                                </td>
    ...
    <div class="mt-8">
        <h2 class="text-xl font-bold text-white mb-4">Blackout Dates</h2>
        <p class="text-gray-400 mb-4">Block specific dates when you're unavailable for bookings (holidays, vacations, etc.)</p>
        ...
        <div class="bg-gray-800 rounded-lg shadow-lg p-6 mb-6">
            <h3 class="text-lg font-semibold text-white mb-4">Add Blackout Date</h3>
        ...
        <div class="bg-gray-800 rounded-lg shadow-lg p-6">
            <h3 class="text-lg font-semibold text-white mb-4">Current Blackout Dates</h3>
            
            @if($blackoutDates->isEmpty())
                <p class="text-gray-400 text-center py-8">No blackout dates set. All days follow your availability schedule.</p>
            @else
                <div class="space-y-2">
    ...
    <div class="mt-6 bg-blue-900/30 border border-blue-700 rounded-lg p-4">
        <h3 class="font-semibold text-blue-300 mb-2">How it works:</h3>
