...
<div class="px-4 sm:px-6 lg:px-8 py-6">
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-100">File Management</h2>
        <p class="text-sm text-gray-400 mt-1">Overview of all client files and document management</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        <div class="bg-gray-800 border border-gray-700 rounded-lg p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-100 mt-1">{{ \App\Models\ClientFile::count() }}</p>
                </div>
        ...
        <div class="bg-gray-800 border border-gray-700 rounded-lg p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-400">Public Files</p>
                    <p class="text-2xl font-bold text-gray-100 mt-1">{{ \App\Models\ClientFile::where('is_public', true)->count() }}</p>
                </div>
        ...
        <div class="bg-gray-800 border border-gray-700 rounded-lg p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-400">Private Files</p>
                    <p class="text-2xl font-bold text-gray-100 mt-1">{{ \App\Models\ClientFile::where('is_public', false)->count() }}</p>
                </div>
        ...
        <div class="bg-gray-800 border border-gray-700 rounded-lg p-6">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-400">Clients with Files</p>
                    <p class="text-2xl font-bold text-gray-100 mt-1">{{ \App\Models\Client::has('files')->count() }}</p>
                </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-{{ $usingCloudStorage ? '2' : '3' }} gap-6 mb-6">
        ...
        <div class="bg-gray-800 border border-gray-700 rounded-lg p-6">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    <p class="text-sm text-gray-400">Storage Type</p>
                    <p class="text-2xl font-bold text-gray-100 mt-1">
                    ...
                    @if($usingCloudStorage)
                        <p class="text-xs text-gray-500 mt-1">S3-compatible bucket</p>
                    @endif
                </div>
        ...
        <div class="p-6">
                    ...
                    <table class="min-w-full divide-y divide-gray-700">
                        <thead class="bg-gray-900">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Total Files</th>
                                ...
                                <tr class="hover:bg-gray-700/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center">
                                            ...
                                            <span class="font-semibold text-gray-100">{{ $client->files_count ?? 0 }}</span>
                                            <span class="ml-1 text-sm text-gray-400">{{ Str::plural('file', $client->files_count ?? 0) }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-300">{{ $client->updated_at->format('M j, Y') }}</div>
                                        <div class="text-xs text-gray-500">{{ $client->updated_at->diffForHumans() }}</div>
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="inline-flex gap-2">
                                            <a href="{{ route('clients.show', $client) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-gray-400 hover:text-gray-200 hover:bg-gray-700/50 rounded-lg transition-colors">
            ...
            @else
                <div class="text-center py-16">
                    ...
                    <h4 class="text-xl font-semibold text-gray-300 mb-2">No clients found</h4>
                    <p class="text-gray-400 mb-6">Clients will appear here once created.</p>
    ...
    <div class="mt-8 bg-gray-900 border border-gray-800 rounded-lg shadow">
        <div class="p-6">
            <div class="flex items-center justify-between mb-6">
                <div>
                    <h3 class="text-xl font-bold text-gray-100">Admin User Files</h3>
                    <p class="text-sm text-gray-400 mt-1">Files uploaded by admin users not associated with specific clients</p>
                </div>
            ...
            <div id="adminUploadForm" class="hidden mb-6 bg-gray-800 border border-gray-700 rounded-lg p-6">
                <h4 class="text-lg font-semibold text-gray-100 mb-4">Upload File for {{ auth()->user()->name }}</h4>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input 
                                type="file" 
                                name="file" 
                                class="block w-full text-gray-200 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-medium file:bg-indigo-600 file:text-white hover:file:bg-indigo-500 file:cursor-pointer border border-gray-700 rounded-lg bg-gray-900/50 cursor-pointer" 
                                required 
                            />
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-6">
                                <label class="flex items-center cursor-pointer group">
                                    <input type="radio" name="is_public" value="0" checked class="w-4 h-4 text-indigo-600 bg-gray-700 border-gray-600 focus:ring-indigo-500">
                                    <span class="ml-2 text-sm text-gray-300 group-hover:text-gray-100">
                                        <i class="fas fa-lock text-yellow-500 mr-1"></i>Private
                                    </span>
                                </label>
                                <label class="flex items-center cursor-pointer group">
                                    <input type="radio" name="is_public" value="1" class="w-4 h-4 text-indigo-600 bg-gray-700 border-gray-600 focus:ring-indigo-500">
                                    <span class="ml-2 text-sm text-gray-300 group-hover:text-gray-100">
                        ...
                        <div>
                            ...
                            <textarea 
                                name="description" 
                                rows="3" 
                                class="w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500" 
            ...
            @if($myFiles->count() > 0)
                <div class="mb-6 bg-gray-800 border border-gray-700 rounded-lg p-4">
                    <div class="flex items-center justify-between mb-3">
                        <h4 class="text-sm font-semibold text-gray-100">Your Recent Files</h4>
                            ...
                            <div class="flex items-center justify-between p-3 bg-gray-700/50 rounded-lg hover:bg-gray-700 transition-colors">
                                <div class="flex items-center flex-1 min-w-0">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-medium text-gray-100 truncate">{{ $file->original_name }}</p>
                    ...
                    <table class="min-w-full divide-y divide-gray-700">
                        <thead class="bg-gray-900">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Total Files</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Last Upload</th>
                                <th class="px-6 py-3 text-right text-xs font-medium text-gray-300 uppercase tracking-wider">Actions</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-700">
                                ...
                                <tr class="hover:bg-gray-700/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center">
                                            ...
                                            <span class="font-semibold text-gray-100">{{ $user->uploaded_files_count }}</span>
                                            <span class="ml-1 text-sm text-gray-400">{{ Str::plural('file', $user->uploaded_files_count) }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        @if($lastUpload)
                                            <div class="text-sm text-gray-300">{{ $lastUpload->created_at->format('M j, Y') }}</div>
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <h4 class="text-lg font-semibold text-gray-300 mb-2">No admin files found</h4>
                    <p class="text-gray-400 text-sm">Admin users haven't uploaded any standalone files yet.</p>
                </div>
            @endif
        </div>
    </div>
