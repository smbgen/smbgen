...
<div class="py-6 space-y-6">
    ...
    <div class="admin-page-header">
        ...
        <div class="action-buttons flex gap-2">
            ...
            <form action="{{ route('admin.leads.toggle-notifications') }}" method="POST" class="flex items-center">
                @csrf
                <label class="flex items-center gap-3 bg-gray-800 px-4 py-2 rounded-lg border border-gray-700 hover:border-blue-600 transition-colors cursor-pointer">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        @if(isset($totalLeads) && isset($todayLeads) && isset($cmsLeads))
            <div class="card bg-gradient-to-br from-blue-900/50 to-blue-800/30 border-blue-700">
                <div class="p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-blue-300 text-sm font-medium">Total Leads</p>
                            <p class="text-3xl font-bold text-white mt-2">{{ $totalLeads }}</p>
                        </div>
                        <div class="bg-blue-600/30 rounded-full p-4">
            ...
            <div class="card bg-gradient-to-br from-green-900/50 to-green-800/30 border-green-700">
                <div class="p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-green-300 text-sm font-medium">Today's Leads</p>
                            <p class="text-3xl font-bold text-white mt-2">{{ $todayLeads }}</p>
                        </div>
                        <div class="bg-green-600/30 rounded-full p-4">
                            <i class="fas fa-calendar-day text-green-300 text-2xl"></i>
                        </div>
                    </div>
                </div>
            </div>

            <div class="card bg-gradient-to-br from-purple-900/50 to-purple-800/30 border-purple-700">
                <div class="p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-purple-300 text-sm font-medium">CMS Form Leads</p>
                            <p class="text-3xl font-bold text-white mt-2">{{ $cmsLeads }}</p>
                        </div>
                        <div class="bg-purple-600/30 rounded-full p-4">
                            <i class="fas fa-file-alt text-purple-300 text-2xl"></i>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="admin-table">
                ...
                <tbody>
                    ...
                    <tr class="cursor-pointer hover:bg-gray-800/50 transition-colors" 
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-gray-100">{{ $lead->name }}</div>
                        ...
                        <td class="px-6 py-4 max-w-md">
                            @if($lead->message)
                                <div class="text-sm text-gray-300 line-clamp-2" title="{{ $lead->message }}">
                                    {{ Str::limit($lead->message, 100) }}
                                </div>
                            @else
                                <span class="text-xs text-gray-500 italic">No message</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($lead->cmsPage)
                                <span class="px-2 py-1 text-xs font-semibold rounded-full bg-purple-600/20 text-purple-300 border border-purple-600/30">
                                    <i class="fas fa-file-alt mr-1"></i>{{ $lead->cmsPage->title }}
                                </span>
                            @else
                                <span class="px-2 py-1 text-xs font-semibold rounded-full bg-gray-600/20 text-gray-300 border border-gray-600/30">
                                    {{ $lead->source_site ?: 'Direct' }}
                                </span>
                            @endif
                            @if($lead->referer)
                                <div class="text-xs text-gray-500 mt-1 truncate max-w-[150px]" title="{{ $lead->referer }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                            ...
                            <div class="text-xs text-gray-600 mt-1">{{ $lead->created_at->diffForHumans() }}</div>
                        </td>
