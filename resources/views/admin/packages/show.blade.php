    ...
    <div class="flex justify-between items-start flex-wrap gap-4">
        <div>
            <div class="flex items-center gap-3 mb-1">
                <a href="{{ route('admin.packages.index') }}" class="text-gray-600 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 text-sm">
                    <i class="fas fa-arrow-left mr-1"></i>Packages
                </a>
                <span class="text-gray-400 dark:text-gray-600">/</span>
                <span class="text-gray-700 dark:text-gray-300 text-sm">{{ $package->client->name }}</span>
            </div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $package->name }}</h1>
        </div>
        <div class="flex items-center gap-3 flex-wrap">
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.outside="open = false"
                    class="absolute right-0 top-8 z-10 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg shadow-xl py-1 min-w-32">
                    @foreach(['draft', 'ready', 'sent'] as $s)
                        @if($s !== $package->status)
                        <form method="POST" action="{{ route('admin.packages.status', $package) }}">
                            @csrf @method('PATCH')
                            <input type="hidden" name="status" value="{{ $s }}">
                            <button type="submit" class="w-full text-left px-4 py-2 text-sm text-gray-900 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">
                                Mark as {{ ucfirst($s) }}
                            </button>
                        </form>
                        @endif
                ...
                <button type="submit"
                    class="text-xs px-3 py-1.5 rounded-full font-medium border transition-colors
                        {{ $package->portal_enabled
                            ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 border-green-400 dark:border-green-700 hover:bg-green-200 dark:hover:bg-green-900/60'
                            : 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 border-gray-400 dark:border-gray-600 hover:bg-gray-300 dark:hover:bg-gray-600' }}">
    ...
    <div class="border-b border-gray-200 dark:border-gray-700">
        <nav class="flex gap-1 -mb-px">
            ...
            <button @click="activeTab = '{{ $key }}'"
                ...
                {{ $tab['label'] }}
                @if($tab['count'] > 0)
                    <span class="text-xs px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300">{{ $tab['count'] }}</span>
                @endif
            </button>
            @endforeach
        </nav>
                    ...
                    <div class="admin-card-body">
                        <div class="flex items-start justify-between mb-3">
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    <h4 class="text-gray-900 dark:text-gray-100 font-medium leading-tight">{{ $file->display_name }}</h4>
                                    <p class="text-xs text-gray-600 dark:text-gray-500 font-mono">{{ $file->original_name }}</p>
                                </div>
                            </div>
                            ...
                            <button
                                ...
                                class="text-xs px-2 py-1 rounded border transition-colors
                                    {{ $file->portal_promoted
                                        ...
                                        : 'bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-400 border-gray-300 dark:border-gray-600 hover:border-green-500 dark:hover:border-green-700 hover:text-green-700 dark:hover:text-green-400' }}">
                    ...
                    <div class="admin-card-body">
                        <div class="flex items-center justify-between flex-wrap gap-3">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <h4 class="text-gray-900 dark:text-gray-100 font-medium flex items-center gap-2">
                                        {{ $file->display_name }}
                                        @if($file->isIndexFile())
                            ...
                            <div class="flex items-center gap-2">
                                <span class="{{ $file->type_badge_class }} px-2 py-0.5 rounded text-xs">{{ $file->type }}</span>
                                <span class="text-xs text-gray-600 dark:text-gray-400">{{ $file->formatted_size }}</span>
                                @if($file->group_label)
                                    <span class="text-xs px-2 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-400 border border-gray-300 dark:border-gray-600">
                ...
                <div class="admin-card">
                    <div class="admin-card-body flex items-center justify-between flex-wrap gap-3">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h4 class="text-gray-900 dark:text-gray-100 font-medium">{{ $file->display_name }}</h4>
                                <p class="text-xs text-gray-600 dark:text-gray-500 font-mono">{{ $file->original_name }}</p>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
