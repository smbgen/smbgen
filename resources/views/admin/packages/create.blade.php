    ...
    <form method="POST" action="{{ route('admin.packages.review') }}" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1 space-y-5">
                ...
                {{-- Client selector --}}
                <div class="admin-card bg-white dark:bg-gray-800">
                    ...
                    <div class="admin-card-body">
                        <select name="client_id" required
                            ...
                            @foreach($clients as $c)
                                <option value="{{ $c->id }}" @selected(old('client_id', $selectedClient?->id) == $c->id)>{{ $c->name }} ({{ $c->email }})</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="admin-card-body space-y-3">
                        <label class="flex items-start gap-3 cursor-pointer group">
                            <input type="radio" name="upload_type" value="zip" x-model="uploadType"
                                class="mt-1 text-blue-500 bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 focus:ring-blue-500">
                            <div>
                                <div class="text-gray-900 dark:text-gray-100 font-medium group-hover:text-gray-700 dark:group-hover:text-white">ZIP Upload</div>
                                <div class="text-xs text-gray-600 dark:text-gray-400">Google Drive export or agent output ZIP</div>
                            </div>
                        </label>
                        <label class="flex items-start gap-3 cursor-pointer group">
                            <input type="radio" name="upload_type" value="multi" x-model="uploadType"
                                class="mt-1 text-blue-500 bg-white dark:bg-gray-700 border-gray-300 dark:border-gray-600 focus:ring-blue-500">
                            <div>
                                <div class="text-gray-900 dark:text-gray-100 font-medium group-hover:text-gray-700 dark:group-hover:text-white">Individual Files</div>
                                <div class="text-xs text-gray-600 dark:text-gray-400">Drag & drop or select multiple files</div>
                            </div>
                        </label>
                    </div>
                </div>
            ...
            <div class="lg:col-span-2">
                ...
                {{-- ZIP upload --}}
                <div x-show="uploadType === 'zip'" class="admin-card bg-white dark:bg-gray-800 h-full">
                    <div class="admin-card-header">
                        <h3 class="admin-card-title"><i class="fas fa-file-archive mr-2 text-yellow-400"></i>ZIP File</h3>
                        <span class="text-xs text-gray-600 dark:text-gray-400">Max 50 MB</span>
                    </div>
                    <div class="admin-card-body">
                        <div class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-10 text-center hover:border-blue-500 dark:hover:border-blue-400 transition-colors"
                             @dragover.prevent @drop.prevent="handleZipDrop($event)">
                            <i class="fas fa-file-archive text-4xl text-yellow-500 mb-4"></i>
                            <p class="text-gray-700 dark:text-gray-300 mb-2" x-text="zipFileName || 'Drag your ZIP here or click to browse'"></p>
                            <p class="text-xs text-gray-600 dark:text-gray-400 mb-4">Folder structure is preserved as group labels</p>
                ...
                {{-- Multi-file upload --}}
                <div x-show="uploadType === 'multi'" class="admin-card bg-white dark:bg-gray-800 h-full">
                    <div class="admin-card-header">
                        <h3 class="admin-card-title"><i class="fas fa-copy mr-2 text-green-400"></i>Individual Files</h3>
                        <span class="text-xs text-gray-600 dark:text-gray-400">Max 50 MB per file</span>
                    </div>
                    <div class="admin-card-body">
                        <div class="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-10 text-center hover:border-blue-500 dark:hover:border-blue-400 transition-colors"
                            ...
                            <template x-if="multiFileNames.length === 0">
                                <p class="text-gray-700 dark:text-gray-300 mb-2">Drag files here or click to browse</p>
                            </template>
                            <template x-if="multiFileNames.length > 0">
