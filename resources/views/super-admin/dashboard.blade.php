...
<div class="max-w-7xl mx-auto py-6 space-y-6">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
            ...
            <h1 class="mt-2 font-semibold text-white text-2xl sm:text-3xl">Super Admin Console</h1>
        ...
        <div class="flex flex-col sm:flex-row gap-3">
            <a href="{{ route('super-admin.guided-setup') }}" class="inline-flex items-center justify-center rounded-lg border border-slate-700 px-4 py-2 text-sm font-medium text-slate-200 hover:bg-slate-800 transition-colors">Guided Setup</a>
            <a href="{{ route('super-admin.deployment-console') }}" class="inline-flex items-center justify-center rounded-lg bg-cyan-500 px-4 py-2 text-sm font-medium text-slate-950 hover:bg-cyan-400 transition-colors">Deployment Console</a>
        </div>
    </div>
    ...
    <div class="admin-card">
        <div class="admin-card-header">
            ...
            <a href="{{ route('super-admin.tenants.index') }}" class="btn-secondary text-sm text-center w-full sm:w-auto">Manage Tenants</a>
        </div>
        <div class="admin-card-body">
            ...
            @else
                <div class="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-4 flex flex-col sm:flex-row sm:items-center gap-3 sm:justify-between">
    ...
    <div class="admin-card">
        ...
        <div class="admin-card-body space-y-4">
            @foreach($modules as $module)
                <div class="flex flex-col sm:flex-row sm:items-start gap-3 sm:justify-between rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                    <div>
                    ...
                    <span class="self-start rounded px-2.5 py-1 text-xs font-medium {{ $module['enabled'] ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' : 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300' }}">{{ $module['enabled'] ? 'Enabled' : 'Disabled' }}</span>
                </div>
            @endforeach
        </div>
    </div>
