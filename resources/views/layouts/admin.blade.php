        ...
        <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-72 border-r transform transition-transform duration-300 -translate-x-full lg:static lg:translate-x-0 lg:relative flex flex-col" style="background-color: var(--bg-primary); border-color: var(--border-color);">
            ...
            <nav class="sidebar-scroll flex-1 overflow-y-auto px-4 py-6">
                ...
                <x-navigation.collapsible-section id="nav_main" label="Main" :defaultOpen="true">
                    ...
                    <x-navigation.sidebar-link :href="route('admin.packages.index')" :active="request()->routeIs('admin.packages.*')" icon="fas fa-box-open" label="Packages" />
                ...
                <x-navigation.collapsible-section id="nav_business" label="Business">
                    <x-navigation.sidebar-link :href="route('clients.import.index')" :active="request()->routeIs('clients.import.*')" icon="fas fa-file-upload" label="Import Clients" />
                    @if(config('business.features.billing'))
                        <x-navigation.sidebar-link :href="route('admin.billing.index')" :active="request()->routeIs('admin.billing.*')" icon="fas fa-file-invoice-dollar" label="Billing & Invoices" />
                    @endif
                ...
                <!-- Extreme Section (Collapsible) -->
                @if(\Route::has('cleanslate.billing.plans'))
                    <x-navigation.collapsible-section id="nav_extreme" label="Extreme">
                        <x-navigation.collapsible-item label="Admin" icon="fas fa-shield-admin">
                            <x-navigation.sidebar-link :href="route('admin.cleanslate.index')" :active="request()->routeIs('admin.cleanslate.*')" icon="fas fa-users" label="Customers" />
                        </x-navigation.collapsible-item>

                        <x-navigation.collapsible-item label="My Account" icon="fas fa-user-circle">
                            <x-navigation.sidebar-link :href="route('cleanslate.entry')" :active="request()->routeIs('cleanslate.*')" icon="fas fa-shield-halved" label="Dashboard" />
                            <x-navigation.sidebar-link :href="route('cleanslate.billing.plans')" :active="request()->routeIs('cleanslate.billing.*')" icon="fas fa-credit-card" label="Plans & Billing" />
                        </x-navigation.collapsible-item>
                    </x-navigation.collapsible-section>
                @endif
