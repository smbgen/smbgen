        ...
        <form method="POST" action="{{ route('password.email') }}" class="space-y-3">
            ...
            <div x-data="{ isGmail: {{ old('email') && preg_match('/@(gmail\.|googlemail\.)/i', old('email')) ? 'true' : 'false' }} }"
                ...
                <div x-show="isGmail"
                    ...
                    <span>Gmail accounts sign in with Google — no password needed. Use <a href="{{ route('auth.google.redirect') }}" class="font-semibold text-blue-600 dark:text-blue-200 underline hover:text-blue-800 dark:hover:text-white">Continue with Google</a> below instead.</span>
                </div>
            </div>
