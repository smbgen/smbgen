                    ...
                    <table class="min-w-full divide-y divide-gray-700">
                        ...
                        <tbody class="bg-gray-800 divide-y divide-gray-700">
                            @foreach($messages as $message)
                                <tr class="{{ ($message->thread_unread_count ?? 0) > 0 ? 'bg-yellow-900/20 border-l-4 border-yellow-500' : 'hover:bg-gray-700' }} transition-colors duration-200 cursor-pointer" onclick="window.location='{{ route('messages.show', $message) }}'">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <div>
                                                <div class="font-medium text-gray-100">{{ $otherPerson->name ?? 'Unknown' }}</div>
                                                <div class="text-sm text-gray-400">{{ $otherPerson->email ?? 'N/A' }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2 mb-1">
                                            <div class="font-medium text-gray-100">{{ $message->subject ?? 'No Subject' }}</div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-gray-100">{{ $message->created_at ? $message->created_at->format('M j, Y') : 'N/A' }}</div>
                                        <div class="text-sm text-gray-400">{{ $message->created_at ? $message->created_at->format('g:i A') : '' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
