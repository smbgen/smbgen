...
{{-- ── PARTNER SPOTLIGHT ─────────────────────────────────────────────── --}}
<section class="bg-slate-900 py-20 px-6" style="border-top: 1px solid rgba(255,255,255,0.05);">
    <div class="max-w-6xl mx-auto">

        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-11">
            <div>
                <span class="text-slate-500 text-xs font-black uppercase tracking-[0.25em] mb-2 block">Partner Spotlight</span>
                <h2 class="text-3xl md:text-4xl font-black text-white tracking-tight">See it in the wild.</h2>
            </div>
            <a href="{{ route('partners.l7') }}" class="shrink-0 text-sm font-bold text-slate-400 hover:text-white transition-colors flex items-center gap-1.5">
                All partners &rarr;
            </a>
        </div>

        <a href="{{ route('partners.l7') }}" class="group block rounded-2xl overflow-hidden transition-all hover:-translate-y-0.5 duration-200"
           style="background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.07);">
            <div class="h-1.5" style="background: linear-gradient(90deg, #16a34a, #4ade80, #3b82f6);"></div>
            <div class="p-8 md:p-10 grid md:grid-cols-2 gap-10 items-center">
                ...
                <div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach(['SIGNAL', 'RELAY', 'SURGE', 'CAST', 'VAULT'] as $product)
                            <span class="text-[10px] font-black uppercase tracking-widest text-green-400/70 px-2.5 py-1 rounded-lg"
                                  style="background: rgba(34,197,94,0.08); border: 1px solid rgba(34,197,94,0.15);">{{ $product }}</span>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="grid grid-cols-3 gap-3">
                    @foreach([
                        ['3',    'Portfolio brands', 'text-green-400',  'rgba(34,197,94,0.08)',   'rgba(34,197,94,0.2)'],
                        ['5',    'Active modules',   'text-blue-400',   'rgba(59,130,246,0.08)',  'rgba(59,130,246,0.2)'],
                        ['100%', 'AI-powered',       'text-violet-400', 'rgba(139,92,246,0.08)',  'rgba(139,92,246,0.2)'],
                    ] as [$num, $label, $textColor, $bg, $border])
                        <div class="rounded-xl p-4 text-center" style="background: {{ $bg }}; border: 1px solid {{ $border }};">
                            <div class="text-2xl font-black {{ $textColor }} mb-1">{{ $num }}</div>
                            <div class="text-slate-600 text-[10px] font-medium leading-tight">{{ $label }}</div>
                        </div>
                    @endforeach
                    <div class="col-span-3 rounded-xl px-5 py-4 flex items-center justify-between"
                         style="background: rgba(34,197,94,0.05); border: 1px solid rgba(34,197,94,0.12);">
                        <span class="text-slate-500 text-xs">Spatio &bull; The Illustrated Live &bull; Military Talent Pipeline</span>
                        <span class="text-green-400 text-sm font-bold group-hover:translate-x-0.5 transition-transform">&rarr;</span>
                    </div>
                </div>
