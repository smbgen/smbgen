...
<tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors" 
    ...
    <td class="px-4 py-4 w-12" onclick="event.stopPropagation()">
        <input type="checkbox" 
               class="client-email-checkbox w-4 h-4 text-blue-600 bg-gray-100 dark:bg-gray-700 border-gray-300 dark:border-gray-600 rounded focus:ring-blue-500 focus:ring-2 cursor-pointer"
    ...
    <td class="px-6 py-4 cursor-pointer" onclick="window.location='{{ route('clients.show', $client) }}'">
        ...
        @if($client->notes && !empty(trim($client->notes)))
            <div class="text-sm text-gray-500 dark:text-gray-400 max-w-xs truncate">{{ Str::limit(trim($client->notes), 50) }}</div>
        @endif
    </td>
    <td class="px-6 py-4 cursor-pointer" onclick="window.location='{{ route('clients.show', $client) }}'">
        <div class="text-gray-900 dark:text-gray-100">{{ $client->email }}</div>
    </td>
    <td class="px-6 py-4 cursor-pointer" onclick="window.location='{{ route('clients.show', $client) }}'">
        <div class="text-gray-900 dark:text-gray-100">{{ $client->phone ?? 'N/A' }}</div>
    </td>
    <td class="px-6 py-4 cursor-pointer" onclick="window.location='{{ route('clients.show', $client) }}'">
    ...
    <td class="px-6 py-4 cursor-pointer" onclick="window.location='{{ route('clients.show', $client) }}'">
        <div class="text-gray-900 dark:text-gray-100">{{ $client->created_at->format('M j, Y') }}</div>
    ...
    @if(config('business.features.file_management'))
        <td class="px-6 py-4" onclick="event.stopPropagation()">
            <a href="{{ route('admin.client.files', $client) }}" class="inline-flex items-center text-sm text-gray-600 dark:text-gray-300 hover:text-blue-400 transition-colors group">
