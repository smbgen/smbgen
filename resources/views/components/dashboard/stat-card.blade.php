...
<div class="group relative overflow-hidden bg-gradient-to-br {{ $card['gradient'] }} rounded-2xl p-6 shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 cursor-pointer"
    ...
    <div class="relative">
        <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-white/80 text-sm font-medium">{{ $card['subtitle'] }}</span>
        </div>
        <div class="text-white text-4xl font-bold mb-2 tabular-nums" x-text="count"></div>
        <div class="text-white/90 text-sm font-medium">{{ $card['title'] }}</div>
        @if(isset($card['route']))
            <a href="{{ route($card['route']) }}" class="mt-4 inline-flex items-center text-white text-sm font-medium hover:text-white/80 transition-colors group/link">
        ...
        @elseif(isset($card['anchor']))
            <a href="{{ $card['anchor'] }}" class="mt-4 inline-flex items-center text-white text-sm font-medium hover:text-white/80 transition-colors group/link">
                {{ $card['linkText'] }} <i class="fas fa-arrow-right ml-2 group-hover/link:translate-x-1 transition-transform"></i>
            </a>
        @endif
    </div>
