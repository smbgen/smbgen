...
<button
    ...
    x-init="init()"
    @click="toggle()"
    :title="darkMode ? 'Currently in dark mode' : 'Currently in light mode'"
    :aria-label="darkMode ? 'Currently in dark mode' : 'Currently in light mode'"
    class="{{ $class }}"
    {{ $attributes }}
>
    <!-- Sun Icon (shown in light mode) -->
    <svg x-show="!darkMode" class="{{ $iconClass }}" fill="currentColor" viewBox="0 0 20 20">
    ...
    <span x-show="!darkMode" class="hidden sm:inline">Light</span>
    <span x-show="darkMode" class="hidden sm:inline">Dark</span>
</button>
